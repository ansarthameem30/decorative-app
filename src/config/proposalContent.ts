  ...
  gameStars: [
    {
      id: 1,
      title: "Your Smile",
      note: "I promise to spend my days looking for new reasons to make your eyes crinkle with happiness."
    },
    {
      id: 2,
      title: "Midnight Talks",
      note: "I promise to always listen—to your dreams, your fears, and your quietest 3 AM thoughts."
    },
    {
      id: 3,
      title: "Safe Sanctuary",
      note: "Whenever the world outside feels heavy, I promise to be your calm and steady shelter."
    },
    {
      id: 4,
      title: "Wild Adventures",
      note: "I promise to explore every new road with you, never letting our spark or curiosity fade."
    },
    {
      id: 5,
      title: "Gentle Patience",
      note: "As we learn each other through all the days ahead, I promise to love you with endless patience."
    },
    {
      id: 6,
      title: "Unfiltered Joy",
      note: "I promise to dance with you in the kitchen, sing off-key, and cherish all our silly moments."
    },
    {
      id: 7,
      title: "Borahae Forever",
      note: "Seven billion lights across the universe, but I will choose you in this lifetime and every one after."
    }
  ],
...
export const getStoredProposalConfig = (): ProposalConfig => {
  if (typeof window === 'undefined') return defaultProposalConfig;
  try {
    const saved = localStorage.getItem('proposal_config');
    if (saved) return JSON.parse(saved);
  } catch (e) {
    console.error('Failed to parse proposal config from localStorage', e);
  }
  return defaultProposalConfig;
};

export const saveStoredProposalConfig = (config: ProposalConfig): void => {
  if (typeof window === 'undefined') return;
  try {
    localStorage.setItem('proposal_config', JSON.stringify(config));
  } catch (e) {
    console.error('Failed to save proposal config to localStorage', e);
  }
};
